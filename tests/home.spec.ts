...
test.describe('Home page', () => {
  ...
  test('home page filters area is visible', { tag: ['@homepage'] }, async ({ homePage }) => {
    await expect(homePage.filtersContainer).toBeVisible();
  });

  test('check the sort filter: Price (High - Low)', { tag: ['@homepage', '@filters'] }, async ({ homePage }, testInfo) => {
  ...
  test('check the sort filter: Price (Low - High)', { tag: ['@homepage', '@filters'] }, async ({ homePage }, testInfo) => {
