...
test.describe('Login page', () => {
  ...
  test('login page title is visible', { tag: ['@login'] }, async ({ loginPage }) => {
    await expect(loginPage.loginTitle).toBeVisible();
  });

  test('checking the visibility of errors in required fields by submitting an empty form', { tag: ['@login'] }, async ({ loginPage }, testInfo) => {
  ...
  test('login with unregistered email', { tag: ['@login'] }, async ({ loginPage }, testInfo) => {
    await loginPage.login(REGISTR_USER.unregisteredEmail, REGISTR_USER.validPassword);
  ...
  test('checking the register link', { tag: ['@login'] }, async ({ loginPage, registrationPage }) => {
    await loginPage.registerLink.click();

    await expect(registrationPage.registrTitle).toBeVisible();
  });

  test('checking the forgot your password link', { tag: ['@login'] }, async ({ loginPage, forgotPasswordPage }) => {
